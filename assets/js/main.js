...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updatePaginationControls(page, filteredCards) {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      button.className = `px-4 py-2 mx-1 rounded-lg ${i === page ? 'bg-transparent border border-yellow-600' : 'bg-gray-200 text-gray-900'}`;
  ...
  function initialize() {
    ...
    allButton.classList.add('bg-transparent', 'text-yellow-600', 'border', 'border-yellow-600');
  ...
  filterButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      filterButtons.forEach(btn => btn.classList.remove('bg-blue-600', 'text-yellow-600', 'border', 'border-yellow-600'));
      filterButtons.forEach(btn => btn.classList.add('bg-gray-200', 'text-gray-900'));

      button.classList.add('bg-transparent', 'text-yellow-600', 'border', 'border-yellow-600');
      button.classList.remove('bg-gray-200', 'text-yellow-600');
